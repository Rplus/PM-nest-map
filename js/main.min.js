...
function setMark(report) {
  ...
  var marker = Leaflet.marker([report.lat, report.lng], {
    icon: L.divIcon({
      className: "poke-icon poke-icon-".concat(report.dex),
...
function getData() {
  ...
  Promise.all([fetchJSON(urls.GAS)]).then(function (d) {
    var data = d[0].data;
    console.log(data);
...
_dialog.elm.closeBtn.addEventListener('click', _dialog.close.bind(_dialog)); // init pm names


fetchJSON("".concat(urls.GAS, "&method=get_pm_name")).then(function (d) {
  _dialog.elm.pmList.innerHTML = d.map(function (name, idx) {
    return "<option value=\"".concat(idx + 1, "\" label=\"").concat(idx + 1, " - ").concat(name, "\"></option>");
  }).join('');
});
